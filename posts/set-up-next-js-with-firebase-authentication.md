...
Localhost should already be in there localhosts, if you already have a domain for your app, add that to the authorised domains so you don't forget. I can't tell you the number of times I've launched something and it's broken when live and the amount of time it takes to remember this is the issue. 
...
We'll also need to ```npm install firebase``` to get the fucntions within our Next JS app. 

Next we need to initialise Firebase, we'll d this by creating a firebase.js file. I like to put this in a lib folder. So ```/lib/firebase.js```
...
Now to create an Authorization utility, we can create an auth.js within our utils folder, so ```/utils/auth.js```
